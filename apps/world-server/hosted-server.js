...
const LIVE_STATUS_MAX_AGE_MS = 500;
...
function personalizeWorld(instance, world, identity) {
  ...
  let turn = world.turn;
  try {
    turn = instance.progressStore.decorateTurnSnapshot(world.turn, playerId);
  } catch (error) {
    console.error('[yes-pusher] could not decorate hosted turn status');
  ...
  return {
    ...
    queue,
    auth: {
      requireWallet: Boolean(world.auth?.requireWallet),
      testMode: Boolean(world.auth?.testMode),
...
function statusFromWorld(instance, world, identity) {
  const personalized = personalizeWorld(instance, world, identity);
  const { coins: _coins, toys: _toys, ...status } = personalized;
  return {
    ...status,
...
const bootstrapServer = http.createServer((request, response) => {
  ...
  const unavailable = ['/api/world', '/api/status', '/events'].includes(pathname);
  writeJson(response, failed || (initializing && unavailable) ? 503 : 200, {
    ok: !failed,
    authoritative: false,
    error: failed
...
const OPTIONAL_PATCHES = Object.freeze([
  ...
  './boundary-network-cache-patch.js',
]);
...
try {
  ...
  let liveWorld = instance.publicSnapshot(null);
  let liveWorldAt = Date.now();
  let refreshPromise = null;

  const refreshLiveWorld = async ({ force = false } = {}) => {
    if (!force && Date.now() - liveWorldAt < LIVE_STATUS_MAX_AGE_MS) return liveWorld;
    if (refreshPromise) return refreshPromise;
    refreshPromise = Promise.resolve().then(() => {
      liveWorld = instance.publicSnapshot(null);
      liveWorldAt = Date.now();
      return liveWorld;
    }).catch((error) => {
      console.error('[yes-pusher] lightweight status refresh failed; keeping the last valid boundary');
      console.error(errorText(error));
      return liveWorld;
    }).finally(() => {
      refreshPromise = null;
  ...
  if (!liveWorld.replay && Number(liveWorld.coinCount || 0) === 0) {
    instance.engine.resetMachine();
    await refreshLiveWorld({ force: true });
    startup.recovery = {
      ...(startup.recovery ?? {}),
  ...
  bootstrapServer.on('request', async (request, response) => {
    ...
    if (request.method === 'GET' && (pathname === '/api/world' || pathname === '/api/status')) {
      try {
        ...
        const world = await refreshLiveWorld();
        const payload = pathname === '/api/status'
          ? statusFromWorld(instance, world, identity)
          : personalizeWorld(instance, world, identity);
        writeJson(response, 200, payload, request);
      } catch (error) {
        console.error(`[yes-pusher] ${pathname} response failed`);
        console.error(errorText(error));
        writeJson(response, 500, {
          ok: false,
          authoritative: true,
          error: error instanceof Error ? error.message : 'Could not read the shared machine.',
        }, request);
      }
      return;
    }
    ...
    if (request.method === 'POST' && pathname === '/api/queue/join') {
      try {
        ...
        const result = fastQueueJoin({
          ...
          identity,
          requireWallet: Boolean(liveWorld.auth?.requireWallet),
          requestedCoins: body.coins,
        });
        writeJson(response, result.status, result.payload, request);
        if (result.status === 200) setImmediate(() => { void refreshLiveWorld({ force: true }); });
      } catch (error) {
    ...
    if (request.method === 'POST' && pathname === '/api/queue/leave') {
      try {
        const body = await readJsonBody(request);
        const identity = requestIdentity(instance, request, requestUrl, body);
        if (!identity?.playerId) {
          writeJson(response, 400, { ok: false, error: 'playerId is required' }, request);
          return;
        }
        instance.queue.leave(identity.playerId, { turnRunning: Boolean(liveWorld.prepare || liveWorld.replay) });
        writeJson(response, 200, { ok: true, left: true }, request);
        setImmediate(() => { void refreshLiveWorld({ force: true }); });
      } catch (error) {
        writeJson(response, 400, {
          ok: false,
          error: error instanceof Error ? error.message : 'Could not leave the shared queue.',
        }, request);
      }
      return;
    }

    if (request.method === 'POST' && pathname === '/api/operator/test-setup') {
      try {
        const identity = requestIdentity(instance, request, requestUrl);
        ...
        const world = await refreshLiveWorld({ force: true });
        writeJson(response, 200, {
          ok: true,
          ...result,
          snapshot: personalizeWorld(instance, world, identity),
        }, request);
      } catch (error) {
    ...
    if (request.method === 'GET' && pathname === '/events' && !requestUrl?.searchParams.get('playerId')) {
      writeJson(response, 200, {
        ok: true,
        authoritative: true,
        ready: true,
